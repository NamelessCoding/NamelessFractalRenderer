...
void main()
{
  ...
  //  smallOffset = vec2(0.);
//+jitter()
    vec4 p22 = vec4((clamp(texCoord+smallOffset,0.,1.) * 2.0 - 1.0), 0.0, 1.0);
    ...
    dir = normalize(mat3(invview) * dir);
     ...
     vec4 p22cam = vec4((clamp(vec2(0.5)+smallOffset,0.,1.) * 2.0 - 1.0), 0.0, 1.0);
    //p22.xy += smallOffset*1.;
    
    vec3 dircam =
	  (invproj * p22cam).xyz / (invproj * p22cam).w;
...
/*
    float focaldist = 7.3;
        ...
        vec3 focuspoint = pos + ((dir*focaldist) / dot(dir,dircam)); //these will lie on the focal plane
           ...
           // if(i == 0){
               // keepk = k;
                ...
                if(renderSkyAndSun){
                    ...
                    //albedos = sky(vec3(0.), dir.xzy, normalize(ldir).xzy)*1.* max(dot(normalize(ldir).xzy, vec3(0.,0.,1.)),0.34);
...
vec3 delta =  lastViewPos - viewPos;
    imageStore(rcpos, ivec3((positions+normals*0.+90.0 - floor(viewPos))), vec4(positions, isEM));
    imageStore(rcnorm, ivec3((positions+normals*0.+90.0 - floor(viewPos))), vec4(normals, 1.));
