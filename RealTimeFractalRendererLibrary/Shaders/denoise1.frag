...
float g3x3(vec2 coords, vec2 iResolution) {
  ...
  1.0 / 273.0, 4.0 / 273.0, 7.0 / 273.0, 4.0 / 273.0, 1.0 / 273.0 };
	...
	for (int i = 0; i < 25; i++) {
		...
		ppp += texture2D(var1, ((p + coords2)/iResolution)).x*atrous_kernel_weights[i];
	}
	return ppp;
}
...
hist = 0.;
    ...
    vec4 currVAR = vec4((var/max(pow(w2,1.),0.001)),  texture2D(var1, texCoord).yzw);
    var2 = currVAR;
...
//float g3x3(vec2 coords, vec2 iResolution) {
        ...
        float wrp = exp(-(abs((lum(currrp)) - lum(nextrp)) / (1.71*dll + 0.000001)));
    ...
    for (int i = 0; i < 9; i++) {
		vec2 coords = vec2(float(i % 3) - 1., float(i / 3) - 1.) * mult  * pow(roughness, 0.5) ;
        ...
        if(roughness < 0.5 && texture2D(secondpos, texCoord).w < 0.5){
            ...
            weigth2 *= max(pow(max(dot(normalize(curN), normalize(nextN)),0.0), 6.), sqrt(sqrt(roughness)));
    ...
    //ret = currVAR.xxx;
    colorfog2 = vec4(reflct/max(reflctdiv,0.0001), texture2D(colorfog, texCoord).w);
	den2 = vec4(ret, max(texture2D(den1, texCoord).w-1.,1.));
   // secalb = vec4(shad1/max(shad1we,0.001), texture2D(reflectionAlb, texCoord).w);
