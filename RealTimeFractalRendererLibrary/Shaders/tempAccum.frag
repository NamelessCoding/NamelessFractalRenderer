...
bool outsc = false;
    float isSpec = texture2D(reflAlb, texCoord).w;
	    ...
	    if(ProjectedCoordinates.x > 1. || ProjectedCoordinates.x < 0. || ProjectedCoordinates.y > 1. || ProjectedCoordinates.y < 0.  ){
...
for(int i = 0; i < 25; i++){
        ...
        float wrp = exp(-(abs((lum(currShad)) - lum(nextshads)) / (12.72 + 0.00001)));
