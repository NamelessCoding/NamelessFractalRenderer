...
vec2 find_previous_reflection_position(vec2 projected) {
  ...
  vec4 Projected2 =
      vec4(view_intersection.xyz, 1.); // + vec4(cameraOffset, 0.);
...
bool plane_distance_disocclusion_check(vec3 current_pos, vec3 history_pos,
                                       vec3 current_normal) {
...
bool out_of_frame_disocclusion_check(vec2 coord, vec2 image_dim) {
  ...
  if (any(lessThan(coord, ivec2(0, 0))) ||
      any(greaterThan(coord, image_dim - ivec2(1, 1))))
    return true;
  else
    return false;
}

bool is_reprojection_valid(vec2 coord, vec3 current_pos, vec3 history_pos,
                           vec3 current_normal, vec3 history_normal,
                           float current_mesh_id, float history_mesh_id,
                           vec2 image_dim) {
  ...
  // if (mesh_id_disocclusion_check(current_mesh_id, history_mesh_id)) return
  // false;

  // check if history sample is on the same plane
  if (plane_distance_disocclusion_check(current_pos, history_pos,
                                        current_normal))
    return false;
