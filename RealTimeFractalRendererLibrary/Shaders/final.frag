...
vec3 c1 =texture2D(TAA, (cir*iResolution + vec2(1.,0.))/iResolution).xyz*-1.;
vec3 c2 =texture2D(TAA, (cir*iResolution + vec2(-1.,0.))/iResolution).xyz*-1.;
vec3 c3 =texture2D(TAA, (cir*iResolution + vec2(0.,1.))/iResolution).xyz*-1.;
vec3 c4 =texture2D(TAA, (cir*iResolution + vec2(0.,-1.))/iResolution).xyz*-1.;
c = c + c1 + c2 + c3 + c4;*/
...
float CoC(float aperture, float focallength, float objectdistance, float planeinfocus){
    return abs(aperture * (focallength * (objectdistance - planeinfocus)) / max(objectdistance * (planeinfocus - focallength), 0.0001));
}
...
vec2(0.791559, 0.59771)
  ...
  float getBlurSize(float depth, float focusPoint, float focusScale, float maxmult)
{
	float coc = clamp((1.0 / focusPoint - 1.0 / depth)*focusScale, -1.0, 1.0);
	return abs(coc) * maxmult;
}

vec3 blur3(vec2 p, float centerDepth, float focalLength, vec2 iResolution, inout uint r, bool mn  ) {
    //vec2 iResolution = vec2(viewWidth, viewHeight);
   ...
   /// float cocSizeS =  clamp(CoC(mm, focalLength, currentDepth, centerDepth), 0.0001, 1. );
   // float newCOC(float As, float focalLength, float depth, float depthCenter){
   // float cocSizeS = clamp(newCOC(mm, focalLength, currentDepth, centerDepth), 0., 1.);
    ...
    float cocSizeS = clamp(getBlurSize(currentDepth, centerDepth, focalLength, maxmult)*multw,0.,maxfin);
   ...
   // for(int i = 0; i < 150; i++){
    ...
    //vec2 coords = vec2(cos(round), sin(round))*(floor(float(i/11.))/4.)*cocSizeS;
   // vec2 cir = (p + coords) / iResolution.xy;
...
vec2 uv2 = sampCoords;
    ...
    for(int is = 0; is < 10; is++){
       vec2 offset = sampCoords*iResolution + offset2*smoothstep(0.,15.-length(uv2*2.0-1.)*1.5+dist, float(is)/20.)*1.;
       rad2.x += texture2D(fogLO, (offset+offset2*0.0064*dist)/iResolution.xy).x;
       rad2.y += texture2D(fogLO, (offset)/iResolution.xy).y;
       rad2.z += texture2D(fogLO, (offset-offset2*0.0064*dist)/iResolution.xy).z;
    ...
    vec3 currCol = rad2;
        ...
        //float CoC(float aperture, float focallength, float objectdistance, float planeinfocus){
        //float CoCSize = clamp(CoC(mm, focalLength, Depth, centerDepth),0.0001,1.);
        //float CoCSize = clamp(newCOC(mm, focalLength, Depth, centerDepth),0.,1.);
    ...
    if(mn){
        ...
        CoCSize = clamp(getBlurSize(Depth, centerDepth, focalLength, maxmult)*multw,0.,maxfin);
    ...
    //return mix(finC, cul,  clamp(exp(-getBlurSize(currentDepth, centerDepth, focalLength)*currentDepth*centerDepth*0.01),0.,1.));
...
//vec3 c = texture2D(colortex5, cir).xyz;
 vec2 uv2 = cir;
    ...
    for(int i = 0; i < 20; i++){
       ...
       rad2.x += texture2D(TAA, (offset+offset2*0.0064*dist)/iResolution.xy).x;
       rad2.y += texture2D(TAA, (offset)/iResolution.xy).y;
       rad2.z += texture2D(TAA, (offset-offset2*0.0064*dist)/iResolution.xy).z;
...
vec3 c2 =  texture2D(TAA, (texCoord*iResolution + vec2(1., 0.))/iResolution).xyz*amp;
vec3 c3 =  texture2D(TAA, (texCoord*iResolution + vec2(-1., 0.))/iResolution).xyz*amp;
vec3 c4 =  texture2D(TAA, (texCoord*iResolution + vec2(0., 1.))/iResolution).xyz*amp;
vec3 c5 =  texture2D(TAA, (texCoord*iResolution + vec2(0., -1.))/iResolution).xyz*amp;
...
for (int i = 0; i < 25; i++) {
    ...
    //accumBloom += textureLod(TAA, newCords, ceil(log2(max(wh.x, wh.y))*lod)).xyz*atrous_kernel_weights[i];
    accumBloom += clamp(texture2D(TAA, newCords).rgb,0., 1.)*atrous_kernel_weights[i];
...
//vec3 blur3(vec2 p, float dist, float centerDepth, float focalLength, vec2 iResolution ) {
...
//vec3 avgBloom = textureLod(fogLO, texCoord, ceil(log2(max(wh.x, wh.y))*0.65)).xyz;
//color2 = mix(color2,avgBloom,0.09);
//float mm = 1.-exp(-0.0001*depth);
//color2 = mix(color2, vec3(10.), mm);
//vec3 upscaleIndirect(vec2 iResolution, vec2 texcc){
...
//color2 = depthOfField(texCoord, depthCenter, 8., 1./wh);
...
col = color2;
//col = texture2D(colortexFog, texCoord).xyz/max(texture2D(colortexFog, texCoord).w,0.1);
vec2 iResolution = wh;
vec3 N = texture2D(normal, texCoord).xyz;
//col = imageLoad(rcpos, ivec3((texture2D(position, texCoord).xyz + 90. - floor(viewPos)))).xyz;

//col = texture2D(normal, texCoord).xyz;


//float expos = texture2D(fogLO, texCoord).w;//fogLO
float avgLuma = pow(textureLod(fogLO, texCoord, ceil(log2(max(wh.x, wh.y)))).a,2.);
...
//col.g *= 1.5;
//col.r *= 1.2;
//col = hsvtorgb(vec3(clamp(col.x,0.,360.),clamp(col.y, 0., 1.),clamp(col.z, 0., 1.)));
//col = vec3(avgLuma);
...
//float sn = texture2D(sunTex, texCoord).b;
  ...
  // col = shadowS(texture2D(position, texCoord).rgb + texture2D(normal, texCoord).xyz*0.1);
