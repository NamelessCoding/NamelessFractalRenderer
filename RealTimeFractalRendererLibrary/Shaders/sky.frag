...
float fbmss(vec3 p)
{
    ...
    float sum = 0.;
        ...
        for(int i = 1; i <= 8; i++)
        {
            sum += texture3D(worl, (p) * scale * pow(2., float(i))).w / pow(1.7, float(i));
        }
...
float Srb = clamp(remap(clamp((cam.z-200.)/15., 0., 0.09), 0., 0.09, 0., 1.),0.,1.);
...
vec3 befo = p;
vec2 tts = boxIntersection(p-vec3(0., 0., 200.), d, vec3(100000., 100000., 0.1));
if(tts.x > 0.){
  p += d*tts.y;
}
...
float density = max(fbm(cam*mult, cam)-minus-abs(cam.z-220.)*0.00014, 0. );
...
//energy = energy*(1.0-rayleighcoefficients);
        ...
        for(int k = 0; k < 10; k++){
            float density2 = max(fbm(cam2*mult, cam2)-minus-abs(cam2.z-220.)*0.00014,0.) ;
...
Ex = Ex*exp(-accum2*0.01)*(1.0-exp(-accum2*max(zz,0.05)*310.8));
...
/*accumulateLight += density * max(Ex, 0.0) * (1.0-exp(-density*0.07)) *
                        (pm2 * exp(-pow(.9,octave) * 0.1 *accum2) * 24.4 * sky2 + pr2  * exp(-.1  *pow(.9,octave) * accum2)*26. 
...
float octave = 0.;
    ...
    for(int i = 0; i < 8; i++){
      //vec3 pm2 = vec3((PM(max(dot(d,lig),0.),0.76)*40.+PM(max(dot(d,lig),0.),pow(0.5, octave)))*12.*max(1.0-zz,0.8));
      vec3 pm2 = vec3((PM(max(dot(d,lig),0.),0.76)))*140. + vec3((PM(max(dot(d,lig),0.),pow(0.5, octave))))*13.5 + vec3((PM(max(dot(d,lig),0.),-0.5)))*40.;
     // pm2 *= mix(vec3(0.9,0.6,0.2), vec3(0.9), max(dot(lig, vec3(0., 0., 1.)), 0.));


      accumulateLight += (1.-exp(-density*0.6))*density*transmission*
      (pm2 + pr2*12.)
      *pow(0.5, octave)*5.*sky2*(exp(-accum2*0.1*pow(.5, octave))*exp(-accum*0.1));
...
void main()
{
    ...
    if (iCoord == bayerFilter[index]  ){
         vec3 d = rayDirection;
      vec3 lig = ldir2.xzy;
      vec3 sky2 = sky(vec3(0.), d, lig);// + exp(-acos(max(dot(d, lig),0.))*100.)*1.;
    ...
    if(dot(d, vec3(0., 0., 1.)) < 0.){
      ...
      vec4 mmm = vec4(0.);
    vec3 sunsh  = vec3(0.);
     mmm = clouds2(vec3(0.,0.,180.),d,normalize(vec3(lig.xy, abs(lig.z))),dist,sunsh);
