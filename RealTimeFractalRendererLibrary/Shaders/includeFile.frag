...
float h(float x){
    return 2.*3.14159*mod(10000.*sin(10000.*x),1.);
}
...
float N2(vec2 xy, float t, float phi){
    ...
    for(int i = 0; i < 30; i++){
        ...
        float b = cos(
        k*((xy.x) - sqrt(5.81/k)*t) + t*phi*h(float(i))
        );
...
float N3(vec2 xy, float t, float phi){
    ...
    for(int i = 0; i < 30; i++){
        ...
        float b =  cos(
        k*((xy.y) - sqrt(5.81/k)*t) + t*phi*h(float(i))
        );
        accum += min(a*abs(b),0.99);
    }
...
float comb(vec2 uv, float t){
    ...
    for(int i = 0; i < 1; i++){
       ...
       // +N3(rot(uv+10.,360.), t,(fbmcc(uv*0.0003+float(i), t, 0.1)));
        accum += N2(rotXX(vec2(1.)-uv, 
        (float(i)/3.)*360.), t+uv.y, 0.1)
...
float fbmss(vec2 p){
  float t = 1.;
return (pow(fbmcc((mod(p*0.5-50., 100.)-50.)+100., t, 0.6),.7))*112.1;
...
float mapPl(vec3 p){
   // p.y -= 1.;
    ...
    p.x *= 0.6;
   p.y *= 1.;
    float plane = p.z + (2.+((sin(p.x + cos(p.y))+cos(p.y+sin(p.x)))*0.03+
...
/*
       vec3 pos = p.xzy;
        const float dLeaves = deTree2( (pos + vec3( 0.0f, 7.0f, 0.0f ))/6. )*6.;
        float sceneDist = dLeaves;
        if ( sceneDist == dLeaves  ) {
            //hitPointSurfaceType = DIFFUSE;
            //const float noiseValue = 0.25f * perlinfbm( vec3( p.xyz ), 0.9f, 6 );
            //hitPointColor = vec3( 0.06f, 0.13f, 0.02f ) * ( 1.0f - noiseValue );
            cccc = vec3(0.2, 0.9, 0.2);
            rough = 0.1;
        }
...
///////////////////////
  ...
  //  float final = min(min(aa,aa),min(aa3, min(sphere, min(sphere2, min(sphere3, sceneDist)))));
...
//float final = min(aa3, sphere);
   // if(final == b2)l = 3.;
    ...
    if(final == aa3){rough = 1.0;cccc=pal(escape333,vec3(0.4,0.4,0.8));}
    if(final == sphere ){rough = 0.01;l = 0.; cccc = vec3(0.9);}
...
float map(vec3 p){
   ...
   // if(final == aa){a2 = 0.1;prob=0.9;c=vec3(0.5);}
       if(final == aa2){rough = 0.1;prob = 0.1;cccc=pal(escape, vec3(0.9,0.9,0.2));
       l=escape*0.00;}
       if(final == aa22){rough = 1.;prob = 0.8;cccc=pal(escape2, vec3(0.2,0.9,0.9));
       l=escape2*0.000;}
...
float map(vec3 p){
   ...
   // if(final == aa){a2 = 0.1;prob=0.9;c=vec3(0.5);}
              ...
              if(final == fr || final == fr2){l=2.;cccc=vec3(cos(p.x*0.1), sin(p.y*0.1), cos(p.z*0.1))*0.5 + 0.5;}
...
//float aa3 = DE((vec3(2.,4.,2.)-p)/3.)*3.;
    ...
    float final = min(min(min(min(min(min(min(min(b2, a),b),c1),c2),c3),mand),mand),mand);
...
float de333 ( vec3 p ) {
	...
	if ( sceneDist == dUpperWall && dUpperWall <= raymarchEpsilon ) {
	 ...
	 float dLightBars = fBoxCheap( p - vec3( 0.0f, 10.0f, 0.0f ), vec3( 10.0f, 0.1f, 1.0f ) );
	...
	if ( sceneDist == dLightBars && dLightBars <= raymarchEpsilon ) {
	 ...
	 float dLightBarHousing = fBoxCheap( p - vec3( 0.0f, 10.15f, 0.0f ), vec3( 11.0f, 0.2f, 1.1f ) );
...
float map(vec3 p){
    ...
    //if(final == c1)c = vec3(0.9,0.1,0.1);
    //if(final == c2)c = vec3(0.1,0.9,0.1);
    if(final == aa||final == aa2){rough = 1.;cccc=pal(escape*2.,vec3(0.8,0.4,0.4));}
    if(final == aa3){rough = 0.1;cccc=vec3(0.9);}
...
float sdSquareFrame(vec3 p) {
    ...
    d = max(d, -(abs(p.y - 13.) - (1.0 - exp(sin(tts * 5. - id * 1.25)*5.)/exp(5.)) * 5.));
    return d;
}
...
float Map(vec3 p)
{
	...
	for(int i = 0; i< 6; i++){
		...
		//Scale and shift
		p=p*Scalexxxx+JC.xyz; dd=dd*Scalexxxx+JC.w;
		p=vec3(1.0,1.0,.92)*p;
...
float map(vec3 p) {
    ...
    if(a < 0.1){
        ...
        if(surf.emission.x > 0.0 || surf.emission.y > 0. || surf.emission.z > 0.){
           // l = 1.;
            cccc = surf.emission;
        }
    }
...
PMMM * 10.  )*mix(vec3(0.9,0.4,0.6), vec3(0.9), clamp(dot(lig, vec3(0., 0., 1.))*4., 0., 1.));
...
//return vec3(0.);
return max(accumulateLight*1.4*pow(clamp(dot(lig, vec3(0., 0., 1.)), 0., 1.),2.),0.) ;
//return vec3(accumulateLight*9999999999999999.06+accumulateLightMie*999999999999999999.);
}
